...
// Automatically creates missing columns to prevent SQL crashes
...
if(isset($_POST['save_overall'])){
    foreach($_POST['marks'] as $student_id => $data){
        ...
        $percentage = ($max_marks > 0) ? ($obtained / $max_marks) * 100 : 0;
...
<div class="container">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            ...
            <small class="text-secondary text-uppercase">Total Marks Mode - Siddipet Node v3.2</small>
        </div>
    ...
    <form method="POST">
        ...
        <div class="glass-card overflow-hidden">
            <div class="p-3 border-bottom border-secondary bg-white bg-opacity-5 d-flex justify-content-between align-items-center">
                <span class="text-uppercase small fw-bold text-secondary">Sheet Editor</span>
                <div class="d-flex gap-2">
                    <input type="number" id="global_max" placeholder="Set Global Max" class="form-control form-control-sm" style="width:140px">
                    <button type="button" class="btn btn-sm btn-info fw-bold" onclick="applyMax()">APPLY TO ALL</button>
                </div>
            </div>
                ...
                <tbody>
                    <?php if($students_data->num_rows > 0): ?>
                        ...
                        <tr>
                            ...
                            <td>
                                <input type="number" 
                                       name="marks[<?= $st['id'] ?>][max_marks]" 
                                       class="form-control mark-input mx-auto max-field" 
                                       value="600" required>
                            </td>
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="5" class="text-center py-5 text-secondary">No students found in this class.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
    ...
    <div class="text-center text-secondary small mt-5">
        &copy; 2026 Ns TECH | Delhi Public Secondary School Siddipet
    </div>
</div>

<script>
/**
 * Quick-fill functionality for Maximum Marks
 */
function applyMax() {
    let val = document.getElementById('global_max').value;
    if(!val) {
        alert('Please enter a value first!');
        return;
    }
    document.querySelectorAll('.max-field').forEach(input => {
        input.value = val;
    });
}
</script>
